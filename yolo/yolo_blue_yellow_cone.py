...
class lidar_cam_cal_Detector():
    def __init__(self, params_cam):
        ...
        self.task = 'detect'
        self.weights = '/home/ha/real/src/rs/src/weights/919_cone_n8_cls.pt'
        self.device = 0
        self.half = False
        self.img_size = 640
        self.conf_thres = 0.7
        ...
        self.point_cloud_blue_pub = rospy.Publisher("/points_inside_box_blue_1", PointCloud2, queue_size=1)
        self.point_cloud_yellow_pub = rospy.Publisher("/points_inside_box_yellow_1", PointCloud2, queue_size=1)
        print("ready...")
    ...
    def img_callback(self, data):
        ...
        if selected_points_inside_box:  # selected_points_inside_box가 비어 있지 않은지 확인
            ...
            points_inside_box_lidar_blue = []
            points_inside_box_lidar_yellow = []
            ...
            for image_x, image_y, depth, c in selected_points_inside_box:
                ...
                if c == "B":
                    points_inside_box_lidar_blue.append((point_lidar[0], point_lidar[1], point_lidar[2]))

                elif c == "Y":
                    points_inside_box_lidar_yellow.append((point_lidar[0], point_lidar[1], point_lidar[2]))
            ...
            pc_data_blue = pc2.create_cloud_xyz32(self.lidar_msg.header, points_inside_box_lidar_blue)
            pc_data_yellow = pc2.create_cloud_xyz32(self.lidar_msg.header, points_inside_box_lidar_yellow)

            if pc_data_blue and pc_data_yellow :
                self.point_cloud_blue_pub.publish(pc_data_blue)
                self.point_cloud_yellow_pub.publish(pc_data_yellow)
        else:
            print("Not data")
            pass

...
class lidar:
    def __init__(self):
        ...
        rospy.Subscriber('/points_inside_box_cone_1', PointCloud2, self.cone_callback)

        self.pub1 = rospy.Publisher("/cluster_cone_1", PoseArray, queue_size=1)
        self.dbscan = DBSCAN(eps=0.2, min_samples=10)

        self.cloud_cone = PointCloud2()
        self.cluster_cone_msg = PoseArray()
        ...
        while not rospy.is_shutdown():
            print(self.is_cone_data)
            if self.is_cone_data:
                start = time.time()
                
                self.cluster_cone_msg.poses = []
                cloud_cone_new = self.pcl_to_ros(self.cloud_cone)  # PCL을 ROS 메시지로 변경
                pc_np_cone = self.pointcloud2_to_xyz(cloud_cone_new)
                
                if len(pc_np_cone) == 0:
                        pass
                
                # cone
                elif len(pc_np_cone) != 0:
                    pc_xy_cone = pc_np_cone[:, :3]
                    db_cone = self.dbscan.fit_predict(pc_xy_cone)
                    n_cluster_cone = np.max(db_cone) + 1
                    self.cluster_cone_msg.header.frame_id = 'map'

                    for c in range(n_cluster_cone):
                        c_tmp = np.mean(pc_xy_cone[db_cone == c, :], axis=0)
                        tmp_pose_cone = Pose()
                        tmp_pose_cone.position.x = c_tmp.tolist()[0]
                        tmp_pose_cone.position.y = c_tmp.tolist()[1]
                        tmp_pose_cone.position.z = c_tmp.tolist()[2]
                        
                        self.cluster_cone_msg.poses.append(tmp_pose_cone)

                    self.pub1.publish(self.cluster_cone_msg)
            
            rate.sleep()
            

    def cone_callback(self, msg):
        self.cloud_cone = self.ros_to_pcl(msg)
        print(self.cloud_cone)

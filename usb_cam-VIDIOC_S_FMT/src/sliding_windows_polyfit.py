...
class ImgParser:
    ...
    def sliding_window(self, frame, search_point):
        ...
        for left_point, right_point in zip(left_line, right_line):
            ...
            mid_path_points_x.append(center_x)
            mid_path_points_y.append(center_y) 
        
        mid_path_points_x.insert(0,320)
        mid_path_points_y.insert(0,240)
        ...
        for i in range(len(x)):
            center_line.append((int(path_func[i]), int(x[i])))

        if len(center_line) > 1:
            pts=np.array(center_line)
            cv2.polylines(frame_color ,[pts] ,False ,(0 ,255 ,0) ,thickness=5)
